...
async def main():
    ...
    session = await session_service.create_session(user_id="console_user", app_name="app")
    ...
    print("===============================================")
    print(f"🔎 Consulta Sanitizada (ctx.state['query']):")
    print(f"   -> {updated_session.state.get('query')}\n")
    
    print(f"🏷️  Categorías Redactadas (ctx.state['redacted_categories']):")
    print(f"   -> {updated_session.state.get('redacted_categories')}\n")
    
    print(f"💬 Respuesta final del Concierge:")
